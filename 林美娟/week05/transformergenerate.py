...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        out = self.drop(attn @ v)                              # [B, n_head, T, d_k]
        out = out.transpose(1, 2).contiguous().view(B, T, H)
        return self.out(out)


class EncoderLayer(nn.Module):
    def __init__(self, hidden, n_head, ff, dropout=0.3):
        super().__init__()
...
if __name__ == "__main__":
    model = TransformerEncoder(hidden=512, n_layer=6, n_head=8, ff=1024, dropout=0.3)
