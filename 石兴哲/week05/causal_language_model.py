"""
字符级语言模型训练脚本，支持 RNN / LSTM / transformer 切换，含 PPL 计算。
用法:
    python language_model.py --model lstm --epochs 20
    python language_model.py --model rnn  --epochs 20
    python language_model.py --model transformer --epochs 30
"""
...
class CharDataset(Dataset):
    ...
    def __len__(self):
        return max(0, len(self.data) - self.seq_len) # 每次滑动一次，滑动的次数；用max来做防御，防止返回负数
...
class LM(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_dim, num_layers, model_type="transformer", num_heads=8, ff=1024, dropout=0.3):
        super().__init__()
        self.embed = nn.Embedding(vocab_size, embed_dim)
        #　换成transformer架构模型 并使用mask
        ...
        # )
        self.encoder = TransformerEncoder(hidden=hidden_dim, n_layer=num_layers, n_head=num_heads, ff=ff, dropout=dropout)
    ...
    def forward(self, x):
        ...
        # out, _ = self.rnn(e)
        T = x.size(1)
        causal_mask = torch.tril(torch.ones(T, T, device=x.device))  # 下三角=1
...
def main():
    ...
    parser.add_argument("--corpus",     default="scratch/data/week05/*.txt")
    parser.add_argument("--save",       default="scratch/data/week05/best_model.pt")
